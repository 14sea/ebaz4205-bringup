...
- **Ethernet is dead**: PHY (IC Plus IP101G, MDIO addr 0, ID `0x02430c54`) responds on MDIO but RJ45 LEDs stay dark and `phydev/link=0` regardless of cable/peer. Verified by swap-test (cable+peer both good against PC). Failure is on the EBAZ-side magnetics/RJ45 (BT16B03 integrated jack). UART is the only way in.
